...
// Memoised: all 445 cards are mounted at once, so any gallery-level state change
    ...
    <div className="group relative transition-transform duration-200 hover:-translate-y-1 focus-within:-translate-y-1">
      <button
        ...
        className={cn(
          ...
          starred
            ? "bg-ink/75 text-amber opacity-100"
            : // Hidden until the card is hovered on a mouse, but always visible on
              ...
              "bg-ink/55 text-white/85 opacity-0 pointer-coarse:opacity-100 hover:bg-ink/75 group-hover:opacity-100",
        )}
      >
